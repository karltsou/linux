...
struct mxt_data {
	...
	u8 suspend_poweroff; //add by josh
...
static int mxt_update_firmware_cfg(struct work_struct *dat);
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	if (status & MXT_T100_DETECT) {
		...
		if(data->debug_log_enabled == 3){
			printk("[Atmel]id= %d, status= 0x%x, type= %d\n",id,status,type);
		}
        //printk("[Atmel]major= %d, pressure= %d, ori= %d\n",message[data->t100_aux_area],message[data->t100_aux_ampl],message[data->t100_aux_vect]);
		switch (type)
		{
		...
		case MXT_T100_TYPE_ACTIVE_STYLUS:
			...
			if (message[6] & MXT_T107_STYLUS_HOVER) {
				...
				/* Stylus detected but no position information */
				touch_active = false;
			}
	...
	if (touch_active) {
		...
		input_report_key(input_dev, BTN_STYLUS, ((message[6] & 0x4)>>2));
		input_report_key(input_dev, BTN_STYLUS2, ((message[6] & 0x8)>>3));
...
static int mxt_update_cfg(struct mxt_data *data, const struct firmware *cfg, int config_check)
{
	...
	if(config_check == 1){
		ret = mxt_cfg_read_version(data, cfg, data_pos);
		if (!ret){
				 ...
				 const struct firmware *cfg, int config_check)
{
	...
	if (cfg) {
		error = mxt_update_cfg(data, cfg, config_check);
		if (error)
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "%u.%u.%02X-%u.%u\n",
			 data->info->version >> 4, data->info->version & 0xf,
			 data->info->build,data->maj_version,data->min_version);
}
...
static int mxt_load_fw(struct device *dev,int fw_path)
{
	printk("[Atmel] %s ++ \n",__func__);
...
static int mxt_self_update_firmware(struct mxt_data *data){
	...
	if((NEWEST_FW_VERSION > data->info->version) ||(NEWEST_FW_BUILD > data->info->build)){
		printk("[Atmel] %s ++ \n",__func__);
		error = mxt_load_fw(dev, 1);
		if (error) {
			dev_err(dev, "The firmware update failed(%d)\n", error);
		} else {
			dev_info(dev, "The firmware update succeeded\n");
...
static int mxt_self_update_cfg(struct mxt_data *data){
	...
	ret = mxt_configure_objects(data, cfg, 1);
...
static int mxt_update_firmware_cfg(struct work_struct *dat)
{
	...
	//mdelay(200);
	ret = mxt_self_update_cfg(touch_chip);
...
{
	printk(KERN_INFO "[Ateml]: touch status=%d. \n",touch_ic_status);
...
static int mxt_touch_sysfs_init(void)
{
	...
	if (ret) 
	{
		printk(KERN_ERR "[Ateml]: create_file debug_level failed\n");
		return ret;
	}
}
...
{ 
	return sprintf(buf, "%u.%u.%02X-%u.%u\n",
		 touch_chip->info->version >> 4, touch_chip->info->version & 0xf,
		 touch_chip->info->build,touch_chip->maj_version,touch_chip->min_version);
}
